      ...
      <nav className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 shadow-sm">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden bg-white border-t shadow-lg">
            <button className="block w-full text-left px-4 py-3 border-b hover:bg-gray-50" onClick={() => { setLocation("/"); setMobileMenuOpen(false); }}>Home</button>
            <button className="block w-full text-left px-4 py-3 border-b hover:bg-gray-50" onClick={() => { setLocation("/products"); setMobileMenuOpen(false); }}>Products</button>
            <button className="block w-full text-left px-4 py-3 border-b hover:bg-gray-50" onClick={() => { setLocation("/cart"); setMobileMenuOpen(false); }}>Cart</button>
            <button className="block w-full text-left px-4 py-3 border-b hover:bg-gray-50 text-blue-600 font-semibold" onClick={() => { setLocation("/customer/login"); setMobileMenuOpen(false); }}>Customer Login</button>
            <button className="block w-full text-left px-4 py-3 hover:bg-gray-50 text-slate-900 font-semibold" onClick={() => { setLocation("/admin/login"); setMobileMenuOpen(false); }}>Admin Login</button>
          </div>
        )}
      </nav>

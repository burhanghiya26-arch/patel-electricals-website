...
export default function AdminSalesmen() {
  ...
  const commissionReport = trpc.salesman.commissionReport.useQuery({ days: null }, { enabled });
  const commissionOrders = trpc.salesman.commissionOrders.useQuery({ days: null }, { enabled });
  ...
  return <div className="min-h-screen bg-background">
    ...
    <main className="container space-y-6 py-8">
      ...
      <Card><CardHeader><CardTitle>Salesman-wise Commission — All Time</CardTitle><CardDescription>Sirf delivered salesman orders ka payable commission.</CardDescription></CardHeader><CardContent>
        <div className="mb-4 rounded-lg bg-emerald-50 p-4"><p className="text-sm text-emerald-800">Total Payable Commission</p><p className="text-2xl font-bold text-emerald-800">₹{Number(commissionReport.data?.totalCommission || 0).toLocaleString("en-IN", { maximumFractionDigits: 2 })}</p></div>
        {!commissionReport.data?.salesmen.length ? <p className="text-sm text-muted-foreground">Abhi delivered orders ka commission nahi bana.</p> : <div className="space-y-3">{commissionReport.data.salesmen.map(person => <div key={person.salesmanId} className="flex flex-col justify-between gap-2 rounded-lg border p-3 sm:flex-row sm:items-center"><div><p className="font-semibold">{person.name}</p><p className="text-sm text-muted-foreground">{person.deliveredOrders} delivered orders · Sale ₹{Number(person.salesAmount).toLocaleString("en-IN")} · Rate {person.commissionRates || "0"}%</p></div><p className="text-lg font-bold text-emerald-700">₹{Number(person.commissionAmount).toLocaleString("en-IN", { maximumFractionDigits: 2 })}</p></div>)}</div>}
      </CardContent></Card>
      <Card><CardHeader><CardTitle>Order-wise Salesman Commission</CardTitle><CardDescription>Har salesman order ka commission. Pending order ka commission delivery ke baad payable banega.</CardDescription></CardHeader><CardContent>
        {commissionOrders.isLoading && <p className="py-5 text-center text-sm text-muted-foreground">Commission orders load ho rahe hain...</p>}
        {!commissionOrders.isLoading && !commissionOrders.data?.length && <p className="text-sm text-muted-foreground">Abhi salesman ke through koi order book nahi hua.</p>}
        <div className="space-y-3">{commissionOrders.data?.map(order => {
          ...
          return <div key={order.id} className="flex flex-col gap-3 rounded-lg border p-3 md:flex-row md:items-center md:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-semibold">{order.orderNumber}</p><span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs capitalize text-slate-700">{order.orderStatus}</span></div><p className="text-sm text-slate-700">{order.salesmanName || "Unknown salesman"} · {order.shopName || order.customerName || "Customer"}</p><p className="text-xs text-muted-foreground">Order: ₹{Number(order.orderAmount || 0).toLocaleString("en-IN")} · Rate: {Number(order.commissionRate || 0)}% · {new Date(order.createdAt).toLocaleDateString("en-IN")}</p></div><div className="shrink-0 md:text-right"><p className="text-lg font-bold">{commission === null ? "Commission not set" : `₹${commission.toLocaleString("en-IN", { maximumFractionDigits: 2 })}`}</p><p className={`text-xs font-medium ${isPayable ? "text-emerald-700" : isCancelled ? "text-red-600" : "text-amber-700"}`}>{isPayable ? "Payable — Delivered" : isCancelled ? "Cancelled — not payable" : "Pending — delivery ke baad payable"}</p></div></div>;
        })}</div>
      </CardContent></Card>
